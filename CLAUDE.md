...
4. **Migraciones**: van en `sam-app/supabase/migrations/`. Normalmente **las corre el
   ...
   la policy RLS exista. 🔴 **Y una COLUMNA nueva tampoco, cuando la tabla tiene
   grants por columna**: `app_usuarios` deja `pin_hash` por fuera a propósito, así que
   `cedula` nació sin `select` → 401 → `loadAppUsers` cayó al espejo de Dexie **sin
   avisar** y el dueño creó dos veces al mismo conductor (4-sep-2026). Fix: `grant
   select (cedula)` — **solo la columna, nunca la tabla entera** (le entregaría el
   `pin_hash` a `anon`). Y un fallback al caché estando EN LÍNEA ya no es mudo.
...
| Planilla | `views/PlanillaTab` | Cuadrícula quincenal + novedades (V, T, F, OV, MT, IN, SP, LL…). 🔴 **El Excel lleva TRES hojas**: la de hectáreas, **Horómetros** (operario × día, la misma forma, para leerlas al lado) y **Horómetros detalle** con las lecturas que no sirven y su motivo. `horasDeLabor()` en `lib/planilla.ts` clasifica cada labor y **devuelve el problema, no lo esconde** — medido en la 1ª quincena de sep-2026, **28 de 170 lecturas dan un número imposible** (`11.698,5 → 1.170.003` = 1.158.304 h en un día). Imprimir eso sumado en la planilla con la que se paga vacía la columna de significado |
...
| Asistencia del taller | `views/taller/MiJornadaTab`, `views/taller/AsistenciaTab`, `lib/horasExtra.ts`, `lib/biometria.ts`, `services/asistenciaApi.ts` | Entrada y salida de los mecánicos **con la CARA** (selfie con prueba de vida en el propio celular, desde el 18-sep-2026: «los dedos normalmente están sucios») o con la huella (WebAuthn), más ubicación, para liquidar horas extras. 🔴 **La cara la compara el SERVIDOR** (`taller_marcar`, migración `20260918160000_taller_rostro.sql`, 24 pruebas en `supabase/pruebas_taller_rostro.sql`): el celular solo saca los 128 números (`lib/rostro.ts`, porte de AgroControl) y NO puede leer las plantillas (grant por columnas). 🔴 **Lo que el celular no puede probar no se paga solo**: parecido dudoso, cara sin aprobar, fuera del taller, sin prueba de vida, subió tarde o sin verificar → `requiere_revision`, no entra a las horas hasta que dueño/administración lo acepte viendo la foto (nadie revisa lo suyo). El registro de la cara pide la **autorización Ley 1581** (texto y versión en `taller_consentimiento_vigente`, se guarda la aceptada) y el jefe lo aprueba viendo la foto; retirar la autorización SUPRIME la plantilla. 🔴 **Entrega HORAS, no pesos**: ocho franjas con su recargo de referencia, y la liquidación la hace nómina. 🔴 **La ley va en una FILA** (`taller_config`), no en el código: nocturno **7 p.m.–6 a.m.** (Ley 2466 de 2025; antes eran las 9 p.m.), **42 h semanales** (Ley 2101, desde 15-jul-2026), dominical al **90%**. Los 18 festivos se **calculan** (Emiliani + Pascua). ⚠️ **Una entrada sin salida NO se cierra sola**: sale en rojo arriba y esas horas no se cuentan. El cliente **solo lee**; todo lo escribe `taller_marcar` (id del teléfono = reintentar no duplica; la hora del teléfono se acota a la del servidor si viene imposible). ⚠️ La firma de WebAuthn **no se verifica en el servidor todavía** — ver `lib/biometria.ts`. Ver `.agent/skills/managing-taller/` |
...
| 🌱 Administración de fincas (MVP) | `views/fincas/*` (incl. `PortalDueno`, `AccesoDueno`, `llaveDueno`, `MapaFinca`, `InstalarApp`, `AvisosFinca`), `supabase/functions/avisos-finca`, `public/sw-avisos.js`, `components/ModoSwitch`, `services/fincasApi`, `lib/fincas` | **Segundo inicio de la app** (22-sep-2026): selector 🚜 Maquinaria \| 🌱 Fincas para owner, administración y supervisor. ASM administra fincas de caña de terceros con transparencia hacia el dueño de la tierra: fincas → suertes → ciclo de corte a corte → paquete de labores con presupuesto y ventana → reporte de campo con foto + GPS + hora del servidor (**funciona sin señal**: cola `outboxFincas` + foto guardada en el equipo) → acepta OTRA persona → gasto en la cuenta del dueño (anticipos, gastos, honorarios, saldo) + resumen por WhatsApp. Las reglas las hace cumplir la base (tablas `af_*`, funciones `af_*`, guardias). 🔴 **Mapa de la finca** (23-sep, RIOGRANDE II): banner + `MapaFinca` (media altura + ⛶ pantalla completa) con pedazos del plano oficial 2024 ubicados sobre el GeoPDF del ingenio (suerte 7 ajustada a la foto). 🔔 **El dueño se entera solo** (25-sep): instala su finca como app (`InstalarApp`; en iPhone la llave queda en la dirección), se actualiza cada 2 min, «novedades desde su última visita» y **avisos push** cuando la maquinaria termina una labor o se acepta un reporte (trigger → `pg_net` → función `avisos-finca` en el VPS → Web Push; llaves VAPID en `af_config.avisos`, fuera del repo) y la maquinaria de ASM pintada por suerte. ⚠️ **Apagados «de momento»: la plata (`VER_PLATA`) y el PIN (`af_config.pin_requerido`)** — ver skill. 🔴 **Acceso directo del dueño** con enlace personal `/#finca=<llave>` (`PortalDueno`): ve SOLO su finca, en vivo y en solo lectura. Las tablas `af_*` están cerradas a la llave pública: todo va por funciones con llave (el personal la saca con su PIN, 30 días; el dueño la trae en el enlace, que solo se guarda como huella y se puede quitar). Ver `.agent/skills/managing-fincas/` |
...
| Eficiencia maquinaria | `views/ConsumoDashboardTab`, `consumoApi.ts` | Segunda cara del tablero del dueño. Une el papel (mar–jul) con la app (ago→) vía `consumo_unificado_v`; gal/hora de CADA máquina contra el **semáforo del cliente** (`semaforo_consumo`, el mismo de Insumos y materiales; desde el 21-sep-2026 — antes era un «▼20% de 5,27» contra la referencia 2025, que sigue en el Excel y al pasar el dedo). En ganchos, ganchos/hora contra el rango de ganchos. Las **horas** salen de `equipo_horas_mes` (cierre mensual de horómetros) y caen a `labor_sesiones` si el mes no tiene cierre |
| Flota · planilla CDA-F-68 | `views/FlotaTab` | El Excel sale **calcado del formato impreso**: membrete IMECOL, códigos de normalización, las 16 columnas y la cuadrícula. ⚠️ Usa **exceljs** y no `xlsx`, porque la versión comunitaria de `xlsx` **no escribe estilos** (probado: descarta bordes y negritas al guardar). Entra por `import()` en su propio chunk. El **conductor también descarga** la suya. 🔴 **Son DOS formatos, no uno con variantes**: además del CDA-F-68 está el **F-OPE-22 «Gestión Operativa» de AgroMorales** (`lib/planillaOpe22.ts`), con número de maquinaria, **km inicial/final/total** y encabezado de FECHA/PLACA/NOMBRES/CÉDULA/SEMANA — sale **una hoja por conductor Y placa**, porque ese encabezado lleva un solo nombre y una sola placa. ⚠️ Antes había un solo campo «total km» tecleado a mano y un conductor lo llenaba con la **lectura del odómetro** (8 de sus 9 servicios: 147.952 km en un viaje de 50 minutos). 🔴 **El F-OPE-22 se llena en DOS FASES** (`FlotaFormOpe22` = Iniciar viaje → `EN_CURSO`; `FlotaCerrarViaje` = km final, firma y foto → `REGISTRADO`, que **sigue siendo el estado completo**): el km total es la resta contra un inicial que nadie tuvo que recordar. El id lo pone el teléfono y el cierre lleva guard `.eq('estado','EN_CURSO')` (reintentar no pisa). Un viaje sin cerrar sale en el Excel marcado «⚠ SIN CERRAR», nunca escondido. 🔴 **Desde el 17-sep-2026 IMECOL TAMBIÉN va en dos fases** («Iniciar servicio» → `EN_CURSO` → «✍️ Terminar y firmar»): el cliente pidió que **solo se firme al terminar el servicio**, porque antes el pasajero firmaba antes de subirse al carro. Y al probarlo apareció que **`mapFlota` convertía `EN_CURSO` en `REGISTRADO`** desde julio: la base guardaba bien el viaje abierto pero la app nunca lo mostró abierto, así que el cierre de AgroMorales **nunca funcionó en pantalla** y Camilo tenía dos viajes atascados. Ver `.agent/skills/managing-flota/` |
...
| Insumos y materiales (bloque) | `views/InsumosCard`, `lib/insumosDash.ts` | ⚠️ **Se monta en UNA sola pantalla**: la cara «📦 Insumos y materiales». Estuvo también en Operación general y el cliente lo mandó quitar (9-sep-2026): esa cara es la de LABORES y esta la del material; la misma tarjeta en las dos obliga a preguntarse cuál es la buena. Filtros de `lib/periodos`: torta de **entregas por material** (cuenta entregas, no cantidades), **combustible por máquina**, **ganchos por máquina**, y el número que une las dos mitades del tablero: **galones por hectárea**. 🔴 **Cada toque segrega un nivel**: torta → material por máquina → lista de entregas → `<DetalleDespacho>`; o máquina → sus materiales → entregas. La lógica vive en la lib **para probarla contra datos reales sin entrar a la sesión** (`import('/src/lib/insumosDash.ts')` en el navegador). ⚠️ El gal/ha **se compara por labor, no por máquina**: TRIPLE ~3,6, SUBSUELO ~3,4, DESPEJE ~0,9 (ago-2026); las PUMA dan 3–5 porque hacen labor pesada. Ver `.agent/skills/managing-movimientos/` |
| Insumos y materiales (tablero) | `views/MovimientosTab`, `movimientosApi`, `resumen_movimientos_insumos` | ⚠️ **Se llama «Insumos y materiales» desde el 8-sep-2026** (lo pidió el cliente); el archivo, la tabla y el RPC siguen diciendo «movimientos» a propósito, igual que con aval/aprobación. Lleva los **mismos filtros de periodo que Operación general** (`lib/periodos`) y, **después de la tira de días**, la misma `<InsumosCard compacta>` con las tres tortas y el gal/ha (`compacta` = sin sus tres cajas, porque los KPI de arriba ya cuentan). 🔴 **Nada se muestra dos veces**: el acordeón que tenía cuatro barras (material por unidad, máquina por combustible) quedó solo con «A quién se entrega», que es lo único que la tarjeta no tiene — el cliente vio las dos versiones juntas y dijo «saturado». Qué se entrega y a quién. 🔴 **La lista por despachador** (entregas, días, por hora, % de registro, cuadre del carro) con su aviso «todavía no son para pagar» **se quitó el 21-sep-2026 a pedido del cliente** («quita esto»); los cálculos siguen en `movimientosApi` por si vuelve el pago por productividad. 🔴 **El volumen nunca se muestra solo**: al lado va la calidad del registro y las visitas. Un despacho es UN hecho — contar filas de kardex infla un 51%. Ver `.agent/skills/managing-movimientos/` |
...
- **🔴 La paleta de los gráficos está SCOPEADA, y fuera de su selector las series salen
  ...
  clase no sale de otro color: no sale — `rgba(0,0,0,0)`, sin error en consola. Ya había pasado
  sin que nadie lo viera: el tablero de insumos (`.mov`) dibujaba sus barras invisibles hasta el
  8-sep-2026. **Al montar gráficas en una pantalla nueva, sumar su clase a ese selector** (la
  maquetación de `.dash` va aparte, para no heredarle el flex a nadie).
...
- **🔴 Galones por hectárea: el denominador es SOLO hectáreas, y la etiqueta lleva la
  labor.** El combustible se carga a una (máquina, día) y la labor no viene en el kardex; el
  puente es ese par, repartiendo el combustible del día a las labores de la máquina por área.
  Dos trampas medidas (ago-2026): **24% de los días-máquina tienen más de una labor**
  (el reparto es aproximado y la pantalla lo dice) y **solo 70% de los días con hectáreas
  tuvieron tanqueo ese día** (un tanqueo alimenta varios días: para «Hoy» el número es
  orientativo, para la quincena se promedia). ⚠️ Un SQL rápido dio PUMA2302 = 2,87 y el
  código da 4,87: el SQL metía los **hectómetros** de ACEQUIAS en el denominador (142 hm) y
  bucketeaba los días en UTC. El código excluye los hm y usa `executionDateKey`/`diaKey`
  (Bogotá). Y la **labor dominante** de una máquina se elige entre labores en ha: «4,87
  gal/ha · ACEQUIAS» mezclaría en la etiqueta lo que el número separa.
...
## 🔴 Probar en el formulario REAL, no el componente suelto (17-sep-2026)

Reclamo textual del cliente: *«esto sigue sin funcionar, pruébalo primero antes de
decirme que ya está»*. Tenía razón: el arreglo del pad de firma se había probado
**montando el componente solo** en el navegador, y funcionó. Dentro del formulario
real había **dos fallas más** que el componente suelto no podía mostrar (el navegador
rechazando la captura del dedo; el recuadro cambiando de ancho al abrirse el modal).
Y probar el flujo entero destapó un tercer defecto que llevaba **dos meses** sin que
nadie lo viera: el mapeo de `EN_CURSO`.

**La regla:** antes de decir «ya está», entrar con un usuario del rol que lo usa
(`U058` es el de pruebas), en **modo celular**, hacer el flujo completo en la pantalla
