...
| Insumos y materiales (Inicio) | `views/InsumosCard`, `lib/insumosDash.ts` | Bloque del tablero de **Operación general**, con sus mismos filtros: torta de **entregas por material** (cuenta entregas, no cantidades), **combustible por máquina**, **ganchos por máquina**, y el número que une las dos mitades del tablero: **galones por hectárea**. 🔴 **Cada toque segrega un nivel**: torta → material por máquina → lista de entregas → `<DetalleDespacho>`; o máquina → sus materiales → entregas. La lógica vive en la lib **para probarla contra datos reales sin entrar a la sesión** (`import('/src/lib/insumosDash.ts')` en el navegador). ⚠️ El gal/ha **se compara por labor, no por máquina**: TRIPLE ~3,6, SUBSUELO ~3,4, DESPEJE ~0,9 (ago-2026); las PUMA dan 3–5 porque hacen labor pesada. Ver `.agent/skills/managing-movimientos/` |
