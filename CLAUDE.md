...
| Asistencia del taller | `views/taller/MiJornadaTab`, `views/taller/AsistenciaTab`, `lib/horasExtra.ts`, `lib/biometria.ts`, `services/asistenciaApi.ts` | Entrada y salida de los mecánicos **con la huella del propio celular** (WebAuthn) más ubicación, para liquidar horas extras. 🔴 **Entrega HORAS, no pesos**: ocho franjas con su recargo de referencia, y la liquidación la hace nómina. 🔴 **La ley va en una FILA** (`taller_config`), no en el código: nocturno **7 p.m.–6 a.m.** (Ley 2466 de 2025; antes eran las 9 p.m.), **42 h semanales** (Ley 2101, desde 15-jul-2026), dominical al **90%**. Los 18 festivos se **calculan** (Emiliani + Pascua). ⚠️ **Una entrada sin salida NO se cierra sola**: sale en rojo arriba y esas horas no se cuentan. El cliente **solo lee**; todo lo escribe `taller_marcar` (id del teléfono = reintentar no duplica; la hora del teléfono se acota a la del servidor si viene imposible). ⚠️ La firma de WebAuthn **no se verifica en el servidor todavía** — ver `lib/biometria.ts`. Ver `.agent/skills/managing-taller/` |
