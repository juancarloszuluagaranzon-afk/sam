...
- **[2026-07-05] ⚠️ "Ha ejecutada" DEBE usar la MISMA fórmula en TODAS las vistas: `executedArea > 0 ? executedArea : area`.** Antes el Resumen ("Por Operador"/"Por Labor"/top) sumaba `executedArea` **crudo** mientras el Reporte usaba el fallback al área planificada → una labor COMPLETADA sin área ejecutada registrada contaba **0 en el Resumen** pero su **área en el Reporte** → descuadre reportado (SARRIA 240.11 vs 266.18). Fix: fallback aplicado en `summaryMetrics`/`summaryByOperator`/`summaryLabor` (SupervisorView), `summarizeAssignments`+`billedArea` (samApi), `operatorMetrics`+Historial (OperatorView), `RealizadasTab`, `FacturacionTab`. **Si agregas una vista que sume ejecutado, usa el mismo `executedArea > 0 ? executedArea : area`.** (Split de cruce-de-día NO se infla: esas COMPLETADA sí traen `executedArea>0`.) El fix de fondo real: garantizar que toda COMPLETADA tenga `executedArea` capturado.
