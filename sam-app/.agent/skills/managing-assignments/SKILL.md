...
- **[2026-06-11]** Nueva pestaña **Planilla** (`PlanillaTab.tsx`, owner+admin): matriz operario (filas, desc por total) × día de la quincena (columnas). La celda suma el **`area`** (planificada, NO `executedArea`) de las labores que el operario **ABRIÓ** ese día (status `EN_PROCESO|PARCIAL|COMPLETADA`), atribuido por `executionDateKey`. Razón del usuario: "se van sumando con las que vaya abriendo" → al ABRIR (poner horómetro inicial) el área entra; lo ejecutado solo crece al cerrar, así que NO sirve. Reusa `matchesSummaryFilter` + `buildMonthOptions` (misma lógica 1ra/2da quincena del Resumen). Acceso: owner vía menú "Más", administración vía nav plana. Tiene descarga a Excel (`import('xlsx')` dinámico, `aoa_to_sheet`). Si piden cambiarla a "ha ejecutadas", cambiar `a.area` → `a.executedArea` y el filtro de status.
