...
**Tipos de novedad agregados después:** `F` Falta sin justa causa (❌, `.planilla-nov--f`), `OV` Oficios varios (🧰), `MT` Máquina en traslado (🚛), `IN` Incapacidad (🩹). **Patrón para agregar una novedad** (sin migración): tocar `NovedadTipo` + `NOVEDAD_TIPOS` (botones) + `ALL_NOVEDAD` (normalizeNovedad) + `NOVEDAD_LABEL` en samApi; `NOV_ICON` en OperatorView; leyenda en PlanillaTab; color `.planilla-nov--xx` en App.css.
