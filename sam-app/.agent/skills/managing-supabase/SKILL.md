...
- **[2026-08-14]** **`CREATE TABLE IF NOT EXISTS` sirve para CREAR, no para EVOLUCIONAR — segunda vez.** Administración no podía crear un ingenio: `column ingenios.activo does not exist`. La migración `20260708120000` SÍ declara `activo`, pero dentro de un `CREATE TABLE IF NOT EXISTS`; la tabla ya existía de antes con otra forma, así que Postgres **saltó el bloque ENTERO —columna incluida— sin error y sin aviso**, y la migración quedó marcada como aplicada con el esquema a medias. Es pariente de la colisión de `labores` (15-jun), pero peor de detectar: allá reventó al insertar, aquí no falló nada hasta que alguien usó la pantalla meses después. **Regla: una columna que se agrega a una tabla que PUEDE existir va SIEMPRE en su propio `ALTER TABLE … ADD COLUMN IF NOT EXISTS`, nunca dentro del CREATE.** Fix: mig. `20260814100000_ingenios_activo_faltante.sql`. ⚠️ Incluir `notify pgrst, 'reload schema'`: sin recargar el esquema, PostgREST sigue sirviendo el viejo y el error persiste aunque la columna ya exista.
