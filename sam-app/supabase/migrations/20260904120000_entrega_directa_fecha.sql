-- La entrega directa puede llevar la fecha en que DE VERDAD ocurrio.
--
-- 🔴 El registro y el hecho no pasan al mismo tiempo: el supervisor entrega a
-- las 6 a.m. en el lote y registra a las 4 p.m. cuando vuelve a tener senal.
-- Hasta hoy `entregar_directo` no recibia fecha, asi que `fecha_efectiva` del
-- kardex tomaba su default (now()) y TODOS los reportes ubicaban la entrega en
-- el momento de teclearla.
--
-- `p_fecha_efectiva` va AL FINAL y CON DEFAULT null (= "usa now()", el
-- comportamiento de siempre). Eso hace segura la ventana de despliegue: una PWA
-- en cache que llame sin ese argumento sigue funcionando igual que antes. Y por
-- la cola offline importa el doble — un registro encolado el martes puede
-- ejecutarse el jueves, y sin este parametro quedaria fechado el jueves.
--
-- ⚠️ `created_at` NO se toca: sigue siendo cuando se tecleo, que es la evidencia
-- que permite detectar a alguien retrofechando. Son DOS fechas y no son
-- intercambiables.

drop function if exists public.entregar_directo(uuid, text, text, text, text, uuid, numeric, text, text, text[], boolean, jsonb);

create function public.entregar_directo(
  p_id uuid, p_operario_id text, p_operario_nombre text, p_despachado_por text,
  p_equipo_codigo text, p_bodega_id uuid, p_horometro numeric, p_ruta text,
  p_nota text, p_evidencia text[], p_engraso boolean, p_items jsonb,
  p_fecha_efectiva timestamptz default null
...
declare
  ...
  -- Cuando OCURRIO. Sin fecha elegida es ahora, como siempre.
  v_ocurrio   timestamptz := coalesce(p_fecha_efectiva, v_ahora);
begin
  ...
  insert into public.insumos_solicitudes (
    ...
    p_id, p_operario_id, p_operario_nombre, p_nota, 'DIRECTA', 'ENTREGADA', v_ocurrio,
  ...
  loop
    ...
    if v_insumo_id is not null and v_cant > 0 then
      ...
      insert into public.insumos_kardex (
        ...
        creado_por, equipo_codigo, fecha_efectiva
      ) values (
        v_insumo_id, v_bodega, 'SALIDA', v_cant, v_saldo, 'Entrega directa',
        p_id::text, p_despachado_por, p_equipo_codigo, v_ocurrio
      );
      -- ⚠️ El stock es el saldo de AHORA, asi que su updated_at es now() y no la
      -- fecha elegida: no se esta diciendo que el inventario cambio ese dia.
...
grant execute on function public.entregar_directo(uuid,text,text,text,text,uuid,numeric,text,text,text[],boolean,jsonb,timestamptz) to anon, authenticated;
